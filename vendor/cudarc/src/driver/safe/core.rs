...
impl<T> Drop for CudaSlice<T> {
    fn drop(&mut self) {
        ...
        // cuMemFreeAsync operates in the calling thread's current context.
        // Event-tracked slices bind through stream.wait() below, but untracked
        // single-stream inference slices have no events to trigger that bind.
        // Bind explicitly so slices remain safe to drop after switching GPUs
        // or moving a model request to another worker thread.
