...
- `vendor/candle-nn` routes GGUF MoE prefill batches of at least 192 tokens through the repaired WMMA kernel; shorter batches use the generic parallel CUDA kernel. The WMMA 64-logical-lane dequantization and shared-memory races were fixed and the path is deterministic, but its F16 activations can produce different long-prompt continuations than the generic Q8_1 activation path. Do not replace either path with token-by-token prompt ingestion.
